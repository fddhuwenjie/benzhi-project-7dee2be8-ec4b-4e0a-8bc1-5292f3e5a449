...
func (t *VigorTrial) Clone() *VigorTrial {
	...
	for i := range t.Deviations {
		...
		copy.Deviations[i].Remediations = append([]RemediationVersion(nil), t.Deviations[i].Remediations...)
		copy.Deviations[i].Decisions = append([]DeviationDecision(nil), t.Deviations[i].Decisions...)
	}
	copy.Audit = append([]AuditEvent(nil), t.Audit...)
